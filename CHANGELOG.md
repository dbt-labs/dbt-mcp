...
* Add `FASTMCP_HOST` and `FASTMCP_PORT` environment variables to configure FastMCP HTTP server binding, enabling external access in Docker/Kubernetes deployments while preserving backward compatibility (defaults to `127.0.0.1:8000`)
### Under the Hood
* Include `settings` in `Config` dataclass for runtime access to FastMCP configuration
* Log configured host and port on startup for container debugging visibility
