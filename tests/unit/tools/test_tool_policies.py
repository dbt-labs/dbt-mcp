...
async def test_tool_policies_match_server_tools():
    ...
    remote_tool_names = {"text_to_sql", "execute_sql"}
    ...
    with default_env_vars_context():
        ...
        # Manually adding remote tools here because the server doesn't get them
        # in this unit test.
        server_tool_names = {tool.name for tool in server_tools} | remote_tool_names
...
def test_tool_policies_match_tool_names():
    policy_names = {policy_name for policy_name in tool_policies}
    tool_names = {tool_name for tool_name in ToolName}
