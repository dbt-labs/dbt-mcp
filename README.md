...
| `DBT_WARN_ERROR_OPTIONS` | `'{"error": ["NoNodesForSelectionCriteria"]}'` | The dbt warnings that should return errors. When using LLMs we likely want to return an error if the dbt command didn't select any node [docs](https://docs.getdbt.com/reference/global-configs/warnings) |
